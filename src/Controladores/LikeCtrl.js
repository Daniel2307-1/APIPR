...
export const obetenerdatosporusuario = async (req, res) => {
  try {
    ...
    const [result] = await sql.query('SELECT *FROM LikeT WHERE id_usuario = ?', [id]);
...
export const postLike = async (req, res) => {
  try {
    ...
    const [result] = await sql.query(
      `INSERT INTO LikeT (id_usuario, id_lenguaje) VALUES (?, ?)`,
      [id_usuario, id_lenguaje]
    );

    res.status(201).json({
      id_favorito: result.insertId,
      message: "Me gusta añadido a Like"
    });
...
export const deleteLike = async (req, res) => {
  try {
    ...
    const [result] = await sql.query(
      'DELETE FROM LikeT WHERE id_usuario = ? AND id_lenguaje = ?',
      [id_usuario, id_lenguaje]
    );
